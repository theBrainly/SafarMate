...
// Webhook-style handler (e.g., Twilio, Meta Cloud API)
// Expected request body:
// { "from": "+2547...", "text": "menu" | "1" | "TRACK s21a" | "SEATS b1", "sessionId": "..." }
export const handleWhatsApp = async (req, res) => {
  const { from, text, sessionId } = req.body || {};
  const input = (text || "").trim();

  const sessionKey = sessionId ? `wa:session:${sessionId}` : from ? `wa:user:${from}` : null;
  let session = {};
  if (sessionKey) {
    try {
  ...
  if (cmd === "1" && !tokens[1]) {
    ...
    if (sessionKey) await client.set(sessionKey, JSON.stringify(session), "EX", 600);
    return res.json({ reply: "Reply with: TRACK <stopId> (e.g., TRACK s21a)" });
  }
  if (session.step === "await_stop" && cmd !== "TRACK") {
    return res.json({ reply: "Please send: TRACK <stopId> (e.g., TRACK s21a)" });
  }
  if (cmd === "TRACK" && tokens[1]) {
    const stopId = tokens[1];
    const routeDoc = await Route.findOne({ "stops.id": stopId });
    if (!routeDoc) return res.json({ reply: "Stop not found." });
    const stop = (routeDoc.stops || []).find((s) => s.id === stopId);
    if (!stop) return res.json({ reply: "Stop not found." });
    const bus = await Bus.findOne({ route_id: routeDoc.id, status: "active" }).lean();
    if (!bus) return res.json({ reply: "No active bus on this route." });
    ...
    return res.json({ reply: `Next bus for ${routeDoc.code} arriving in ${minutesText}. ${seats} seats available.` });
  }

  if (cmd === "2" && !tokens[1]) {
    session = { step: "await_bus" };
    if (sessionKey) await client.set(sessionKey, JSON.stringify(session), "EX", 600);
    return res.json({ reply: "Reply with: SEATS <busId> (e.g., SEATS b1)" });
  }
  if (session.step === "await_bus" && cmd !== "SEATS") {
    return res.json({ reply: "Please send: SEATS <busId> (e.g., SEATS b1)" });
  }
  if (cmd === "SEATS" && tokens[1]) {
    const busId = tokens[1];
    const bus = await Bus.findOne({ id: busId }).lean();
    if (!bus) return res.json({ reply: "Bus not found." });
    const seats = typeof bus.seat_count === "number" ? bus.seat_count : "N/A";
    if (sessionKey) await client.del(sessionKey);
    return res.json({ reply: `Bus ${busId}: ${seats} seats available.` });
  }

  return res.json({ reply: `Unknown command.\n${help}` });
};
